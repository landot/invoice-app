...
describe("invoice reducer", () => {
  ...
  it("should handle editInvoice", () => {
    ...
    const actual = invoiceReducer(
      initialState,
      editInvoice({ invoiceId: invoiceId, updatedInvoice: updatedInvoice }),
    );
  ...
  it("should handle deleteInvoice", () => {
    ...
    expect(actual.value.findIndex((invoice) => invoice.id === invoiceId)).toBe(
      -1,
    );
  });
