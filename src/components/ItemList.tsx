...
import { ItemErrorStyles, ItemListStyles, ItemTableStyles, ItemTableRow, ItemNameCellStyles, ItemQuantityCellStyles, ItemPriceCellStyles, ItemTotalCellStyles, ItemDeleteCellStyles } from "../styles/components/ItemList.styles";
    ...
    return (
        <ItemListStyles>
                        ...
                        <ItemTableRow key={index}>
                            ...
                            <ItemQuantityCellStyles>
                                <TextField 
                                    ...
                                    handleChange={(value) => handleItemUpdate(index, 'quantity', value)}
                                    error={props.showErrors ? item.quantity <= 0 || isNaN(item.quantity): false}
                                    errorText=''
                                />
                            </ItemQuantityCellStyles>
                            <ItemPriceCellStyles>
                                <TextField 
                                    ...
                                    handleChange={(value) => handleItemUpdate(index, 'price', value)}
                                    error={props.showErrors ? item.price <= 0 || isNaN(item.price): false}
                                    errorText=''
                                />
                            </ItemPriceCellStyles>
                            <ItemTotalCellStyles>
            ...
            <ItemErrorStyles>
                ...
                {props.showErrors && items.find(item => item.quantity <= 0 || item.price <= 0) && (
                    <BodyText>- values must be greater than 0</BodyText>
                )}
                {props.showErrors && items.find(item => item.name === '' || isNaN(item.quantity) || isNaN(item.price)) && (
