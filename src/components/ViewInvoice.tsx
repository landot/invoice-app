...
import {
  ...
  TableCellAlignRight,
  AmountDueStyles,
} from "../styles/components/ViewInvoice.styles";
      ...
      <ViewInvoiceBodyStyles>
        ...
        <ViewInvoiceBillingStyles>
          <ViewInvoiceDateStyles>
            <InvoiceSectionStyles>
              ...
              <HeadingSAlt>
                {getDateStringForTimestamp(props.invoice.createdAt)}
              </HeadingSAlt>
            </InvoiceSectionStyles>
            <InvoiceSectionStyles>
              <BodyText>Payment Due</BodyText>
              <HeadingSAlt>
                {getDateStringForTimestamp(props.invoice.paymentDue)}
              </HeadingSAlt>
            </InvoiceSectionStyles>
          </ViewInvoiceDateStyles>
          <ViewInvoiceBillingInfoStyles>
        ...
        <ViewInvoiceItemStyles>
            ...
            <tbody>
              {props.invoice.items.map((item, index) => (
                <tr key={index}>
                  ...
                  <TableCellAlignRight>
                    <BodyText>{convertToCostString(item.price)}</BodyText>
                  </TableCellAlignRight>
                  <TableCellAlignRight>
                    <HeadingS>
                      £ {convertToCostString(item.quantity * item.price)}
                    </HeadingS>
                  </TableCellAlignRight>
                </tr>
              ))}
          ...
          <AmountDueStyles>
            ...
            <HeadingM>£ {getTotalString(props.invoice.items)}</HeadingM>
          </AmountDueStyles>
        </ViewInvoiceItemStyles>
      </ViewInvoiceBodyStyles>
      {size.width && size.width <= 600 && (
